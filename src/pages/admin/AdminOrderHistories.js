        ...
        <div className="h-screen w-full flex flex-col">
            ...
            <div className="flex bg-orange-50">
                ...
                <div className="flex-grow my-4 md:mx-20 mx-4 min-h-screen py-5">
                    ...
                    <div className="mb-4 mt-7 font-bold">
                        Filter Between: 
                        <input type="date" onChange={(e)=> setStartDate(e.target.value)}  className="border border-slate-400 bg-orange-50 p-1 mx-2 rounded-lg"/> 
                        And 
                        <input type="date" onChange={(e)=> setEndDate(e.target.value)} className="border border-slate-400 bg-orange-50 p-1 ml-2 rounded-lg"/>
                        <button onClick={() => handleFilter()} className="bg-orange-200 p-2 hover:bg-orange-300 rounded-lg font-bold text-sm border border-slate-400 ml-2">Filter</button>
                        <button onClick={() => handleDeleteByDate()} className="bg-orange-200 p-2 hover:bg-orange-300 rounded-lg font-bold text-sm border border-slate-400 ml-2">Delete By Date</button>
                    </div>
                    ...
                    <table className="table-auto border border-slate-400 divide-y divide-slate-400 min-w-full shadow-md">
                        ...
                        <tbody className="divide-y divide-slate-400">
                            ...
                            <tr key={item?.id} className="">
                                ...
                                <td className="text-center font-semibold">{item?.user_name}</td>
                                <td className="text-center font-semibold">{item?.grand_weight}</td>
                                <td className="text-center font-semibold">{item?.grand_total}</td>
                                <td className="text-center font-semibold">{item?.user_address}</td>
                                <td className="text-center">{new Date(item?.createdAt).toLocaleString(['ban', 'id'])}</td>
                                <td className="py-3">
